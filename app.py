...
# === Webhook Gumroad principal ===
@app.post("/webhooks/gumroad")
def gumroad_webhook():
    """
    Reçoit l’event Gumroad (POST). Crée un fichier client dans /tenants.
    """
    try:
        # Récupération data JSON ou form
        if request.is_json:
        ...
        # ✅ message vert Render (ANSI green)
...
def api_chat(tenant_id):
    ...
    # Exemple OpenAI (ou Together/Groq)
    try:
        import requests
